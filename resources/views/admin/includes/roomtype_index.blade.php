@foreach ($roomtype as $room)
    <tr>
        <td>
            <div class="custom-control custom-checkbox">
                <input type="checkbox" name="check[]" class="custom-control-input single_select_checkbox" id="single_select_checkbox-{{ $room->id }}">
                <label class="custom-control-label" for="single_select_checkbox-{{ $room->id }}"></label>
            </div>
        </td>
        <td>{{ $loop->index + 1 }}</td>
        <td>{{ $room->name }}</td>
        <td>{{ $room->createdBy->name ?? "N/A"}}</td>
        <td>{{ $room->updatedBy->name ?? "N/A" }}</td>
        ...
        <td>
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right">
                    <a class="dropdown-item" data-toggle="modal" data-target="#roomEdit{{ $room->id }}" title="Edit">
                        <i data-feather='edit'></i>
                        <span>{{ __('Edit') }}</span>
                    </a>

                    <a class="dropdown-item" data-toggle="modal" data-target="#roomDelete{{ $room->id }}" title="Delete">
    ...
    @push('all_modals')
        <!-- Single Edit Modal -->
        <div class="modal fade" id="roomEdit{{ $room->id }}" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title"> {{ __('Room type Update') }} </h5>
        ...
        <div class="modal fade" id="roomDelete{{ $room->id }}" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <form action="{{ route('roomtype.destroy', $room->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('delete')
