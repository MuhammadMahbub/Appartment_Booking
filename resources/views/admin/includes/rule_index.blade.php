@foreach ($rule as $rule)
    <tr>
        <td>
            <div class="custom-control custom-checkbox">
                <input type="checkbox" name="check[]" class="custom-control-input single_select_checkbox" id="single_select_checkbox-{{ $rule->id }}">
                <label class="custom-control-label" for="single_select_checkbox-{{ $rule->id }}"></label>
            </div>
        </td>
        <td>{{ $loop->index + 1 }}</td>
        <td>{{ $rule->name }}</td>
        <td>{{ $rule->createdBy->name ?? "N/A"}}</td>
        <td>{{ $rule->updatedBy->name ?? "N/A" }}</td>
        ...
        <td>
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right">
                    <a class="dropdown-item" data-toggle="modal" data-target="#ruleEdit{{ $rule->id }}" title="Edit">
                        <i data-feather='edit'></i>
                        <span>{{ __('Edit') }}</span>
                    </a>

                    <a class="dropdown-item" data-toggle="modal" data-target="#ruleDelete{{ $rule->id }}" title="Delete">
    ...
    @push('all_modals')
        <!-- Single Edit Modal -->
        <div class="modal fade" id="ruleEdit{{ $rule->id }}" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title"> {{ __('Home Rule Update') }} </h5>
                    ...
                    <form action="{{ route('rule.update', $rule->id) }}" method="POST" enctype="multipart/form-data">
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="name" class="form-control" name="name" value="{{ $rule->name }}" />
                            </div>
                            @error('name')
        ...
        <!-- Single Delete Modal -->
        <div class="modal fade" id="ruleDelete{{ $rule->id }}" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    {{-- <form action="{{ route('rule.destroy', $rule->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('delete') --}}
