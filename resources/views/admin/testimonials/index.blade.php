...
@section('content')
    ...
    @push('all_modals')
    <!-- Create Modal -->
    <div class="modal fade" id="testimonialCreate" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-md modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"> {{ __('Testimonial Create') }} </h5>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="button-group-spacing w-100 text-right">
                        ...
                        <button class="btn btn-warning waves-effect w-100 w-sm-auto" data-toggle="modal" data-target="#testimonialCreate">
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody id="render_testimonial">

                                @include('admin.includes.testimonial_index')
...
<script>
    $(document).ready(function () {
        var ids = [];
        @if($errors->has('name') || $errors->has('designation') || $errors->has('description') || $errors->has('image'))
            $('#testimonialCreate').modal('show');
        @endif

        @if($errors->has('name_edit') || $errors->has('designation_edit') || $errors->has('description_edit') )
            $("#testimonialEdit{{ session('id') }}").modal('show');
        @endif
        ...
        $('#all_select_checkbox').change(function(){
            ...
            if($(this).is(":checked")){
                ...
                $("#delete_all").on('click', function(){
                    ...
                    $.ajax({
                    url: "{{ route('testimonial_multi_delete') }}",
                    type: 'POST',
                    data: {
                        ids: ids,
                    },
        ...
        $('.single_select_checkbox').change(function(){
            ...
            else
            {
                ...
                $("#delete_all").on('click', function(){
                    ...
                    $.ajax({
                        url: "{{ route('testimonial_single_delete') }}",
                        type: 'POST',
                        data: {
                            ids: ids,
                        },
        ...
        $('#filter__date').on('click',function(){
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('date_wise_search_testimonial') }}",
                data: {
                    from_date: from_date,
                    to_date: to_date,
                },
                success: function(response) {
                    ...
                    if ((response.count)*1 <  1) {
                        $('#render_testimonial').html('<tr ><td colspan="1000" class="text-danger text-center py-3">No Data Found</td></tr>');
                    } else {
                        $('#render_testimonial').html(response.data);
                    }
        ...
        $("#clear__filter__date").on("click", function(){
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('date_wise_clear_testimonial') }}",

                success: function(response) {
                    $('#render_rule').html(response.data);
                    window.location.reload();
                }
            })
        });// clear filter
        $("#clear__filter__date").on("click", function(){
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('date_wise_clear_rule') }}",
